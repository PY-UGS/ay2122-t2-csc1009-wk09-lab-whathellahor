...
public class RandomCharacter {
    ...
    public boolean checkPrime(int num){
        boolean flag = true;
        int i = 2;              // Skips 0 and 1
        while(i <= num/2){
            if (num % i == 0){  // Divide the number by incrementing value to check if it is prime
                flag = false;   // Set the flag to be false
                break;          // Break out of the while loop
            }
            ++i;
        }
        return flag;    // Return the boolean value of variable flag
    }

    // Second method for testing prime
    public boolean checkPrime2(int num){
        boolean flag = true;
        for(int i = 2; i <= num/2; ++i){    // Skips 0 and 1
            if(num % i == 0){               // Divide the number by incrementing value to check if it is prime
                flag = false;               // Set the flag to be false
                break;                      // Break out of the for loop
            }
        }       
        return flag;    // Return the boolean value of the variable flag
    }
}